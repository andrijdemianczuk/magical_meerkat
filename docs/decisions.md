...
## D-009: "Who chose the resource", not "does the purpose justify the capability"
- **Decision:** `purpose_capability_mismatch` is replaced by `caller_uncontrolled_resource`: a description routes acquired content into an argument, and the thing acquired is named by a *literal* rather than by one of the tool's declared inputs. No LLM judge in the detection path.
- **Context:** The semantic framing ("does summarizing text justify reading a file?") needs a model. But two of the four fixtures read files and both are legitimate — reading is not the signal. The signal is that a weather tool names `~/.ssh/id_rsa` itself, while `translate_document` reads whatever the caller put in `source_path`. Caller-chosen versus author-chosen is structural and computable. It also generalizes past credentials: a tool exfiltrating a customer list matches no blocklist.
- **Alternatives rejected:** An LLM judge deciding detection (it reads attacker-controlled text, so a successful injection against the judge blinds the scanner, and an outage or rate limit does the same); a domain ontology mapping purposes to permitted capabilities (endless, brittle, and wrong at the edges).
- **Tradeoff accepted:** Two evasions remain, both documented and neither yet weighted: a payload naming no resource at all, and laundering a bare identifier by declaring an input with that name. Paths, URLs, and env vars are immune to the second by construction, and a test pins that.
- **Note:** This was found by *writing the control first*. `translate_document` was flagged at exactly threshold by the previous detector — a live false positive that no payload fixture would ever have surfaced. If a judge is added later it belongs in Phase 2 severity and remediation, where the deterministic path remains the fallback.
